  ...
  if request.method == "POST":
    ...
    try:
      if action == "insert":
        ...
        supplier.supplier_name = request.POST.get("owner_name") or supplier.supplier_name
        supplier.company_name = request.POST.get("company_name") or supplier.company_name
        supplier.email = request.POST.get("sup_email") or supplier.email
        supplier.phone = request.POST.get("sup_mobilenum") or supplier.phone
        supplier.gst_no = request.POST.get("gst_number") or supplier.gst_no
        supplier.address_one = request.POST.get("sup_addresh_primary") or supplier.address_one
        supplier.address_two = request.POST.get("sup_addresh_secondary") or supplier.address_two
    ...
    if action == "insert":
      try:
        with transaction.atomic(): # 1. Guarantee Database Integrity
          brand_obj = Brand.objects.get(brand_id=request.POST.get('brand'))
          ...
          if 'cpu' in cat_name:
              ...
              RAM.objects.create(
                  ...
                  capacity_gb=safe_int(request.POST.get('memory_size')), # Mapped from memory_size
                  speed_mhz=safe_int(request.POST.get('memory_speed')),  # Mapped from memory_speed
              ...
              supported_ff = []
              if request.POST.get('supported_atx'): supported_ff.append("ATX")
              if request.POST.get('supp_micro_atx'): supported_ff.append("Micro-ATX")
              if request.POST.get('supp_mini_itx'): supported_ff.append("Mini-ITX")
              ...
              Motherboard.objects.create(
                  ...
                  chipset=request.POST.get('chipset', 'Generic'), # WARNING: You are missing this input in your HTML!
      ...
      try:
        with transaction.atomic():
          ...
          product.product_name = request.POST.get("name", product.product_name).strip()
          ...
          product.warranty_months = safe_int(request.POST.get("warranty_month", product.warranty_months))
          product.description = request.POST.get("description", product.description).strip()
          product.save()

          # 3. Update Variant Level Data
          variant.variant_name = request.POST.get("v_name", variant.variant_name).strip()
          ...
          variant.price = safe_float(request.POST.get("price", variant.price))
          ...
          if inventory.stock_quantity != new_stock:
              ...
              InventoryLog.objects.create(
                  variant=variant,
                  quantity_change=abs(qty_diff), # Log always takes absolute value
                  action=action_type,
          ...
          category_obj = Category.objects.get(category_id=new_category_id)
          cat_name = category_obj.category_name.lower()
          ...
          if 'cpu' in cat_name:
              ...
              is_modular = request.POST.get('is_modular')
              modularity_choice = 'FULL' if str(is_modular) == '1' else 'NON'
          ...
          if deleted_images:
              ...
              image_ids_to_delete = [int(img_id) for img_id in deleted_images.split(',') if img_id.isdigit()]
              ProductImage.objects.filter(image_id__in=image_ids_to_delete, variant=variant).delete()
          ...
          for img in new_images:
              ...
              is_first = not ProductImage.objects.filter(variant=variant).exists()
              ProductImage.objects.create(
                  variant=variant,
                  image=img,
                  is_primary=is_first 
...
def dashboard_inventory(request):
    ...
    if request.method == "POST":
        ...
        supplier_id = request.POST.get("supplier_id") # Grab the new supplier field
        
        try:
            with transaction.atomic():
                variant = ProductVariant.objects.get(variant_id=variant_id)
                inventory, created = Inventory.objects.get_or_create(variant=variant, defaults={'stock_quantity': 0})
                
                if adj_type == 'add':
                    inventory.stock_quantity += qty
                elif adj_type == 'remove':
                    if inventory.stock_quantity < qty:
                        raise ValueError("Cannot remove more stock than currently available.")
                    inventory.stock_quantity -= qty
    ...
    for inv in inventory_items:
        latest_log = InventoryLog.objects.filter(variant=inv.variant).order_by('-created_at').first()
        img_obj = inv.variant.images.filter(is_primary=True).first() or inv.variant.images.first()
...
def dashboard_per_built(request):
        ...
        try:
            with transaction.atomic():
                if action == "insert":
                    ...
                    build.price = safe_float(request.POST.get("price", build.price))
                    build.description = request.POST.get("description", build.description).strip()
    ...
    context['cpu_list'] = format_variants(ProductVariant.objects.filter(product__category__category_name__icontains='cpu'))
    context['mobo_list'] = format_variants(ProductVariant.objects.filter(product__category__category_name__icontains='motherboard'))
    context['gpu_list'] = format_variants(ProductVariant.objects.filter(product__category__category_name__icontains='gpu'))
    context['ram_list'] = format_variants(ProductVariant.objects.filter(product__category__category_name__icontains='ram'))
  ...
  # ---------------------------------------------------------
    ...
    ssd_variants = ProductVariant.objects.filter(
        ...
        variant_id__in=(m2_ids + hdd_ids)  # Safely exclude the M.2 and HDD items
    ).distinct()
    
    context['ssd_list'] = format_variants(ssd_variants)
    
    context['psu_list'] = format_variants(ProductVariant.objects.filter(product__category__category_name__icontains='psu'))
    context['cabinet_list'] = format_variants(ProductVariant.objects.filter(product__category__category_name__icontains='cabinet'))
...
def get_variant_specs(request, variant_id):
    ...
    """
    variant = ProductVariant.objects.select_related('product__category').filter(variant_id=variant_id).first()

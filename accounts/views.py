...
def send_otp(request):
  if request.method == "POST":
    ...
    if not value:
      messages.error(request, "Please enter your registered email or phone number.")
    ...
    if re.match(email_pattern, value.lower()):
      ...
      messages.error(request, "Enter a valid email address or 10-digit phone number.")
      return render(request, "LoginPage/send_otp.html")
...
def change_password(request):
  ...
  if request.method == "POST":
    ...
    if not user:
      messages.error(request, "Account not found. Please retry password reset.")
      return redirect("accounts:send_otp")
    ...
    if is_same:
      messages.error(request, "Please enter a new password different from the current one.")
      return render(request, "LoginPage/changepassword.html")

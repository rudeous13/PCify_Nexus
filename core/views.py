...
# from builds.models import PCBuild  <-- Adjust 'builds' to whatever app name holds your PCBuild model

def prebuilt(request):
    # Fetch all builds and optimize with prefetch_related to grab all items and their products in one go
    builds_qs = PCBuild.objects.prefetch_related('items__variant__product')

    # 1. Handle Filtering (Budget vs Premium limits set by your UI)
    ...
    # 3. Map data into a clean dictionary for the template
    prebuilt_pcs = []
    for build in builds_qs:
        specs = []
        for item in build.items.all():
            # Dynamic icon assignment based on component type
            ...
            specs.append({
                ...
                # Failsafe in case variant doesn't have a product attached
        ...
        prebuilt_pcs.append({
            ...
            'specs': specs,
            'quick_specs': specs[:3], # We slice this here, saving the template from doing the work
        })
